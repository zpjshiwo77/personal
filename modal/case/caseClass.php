<?php
	...
	function addClassToSql($name,$conn){
		...
		$sql = "INSERT INTO case_class (name) VALUES ('" . $name . "')";
	...
	function judgeSame($name,$conn){
		...
		$sql = "SELECT name FROM case_class WHERE name = '". $name ."'";
	...
	function getClassList(){
		...
		if($page && $page > 0){
			...
			$sql = "SELECT * FROM case_class order by ID desc limit ".($page * 12 ).",12";
			$totalPage = getClassPageTotal();
	...
	function getClassPageTotal(){
		include "../common/connectSQL.php";
		$sqlCount = "SELECT COUNT(*) as count FROM case_class";
	...
	function deleteClass(){
		...
		if($id){
			...
			$sql = "DELETE FROM case_class WHERE ID = " . $id;
